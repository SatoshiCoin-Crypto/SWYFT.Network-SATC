...
void OverviewPage::setBalance(const CAmount& balance, const CAmount& unconfirmedBalance, const CAmount& immatureBalance, const CAmount& anonymizedBalance, const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    ui->label_SATC4->setVisible(showImmature || showWatchOnlyImmature);
...
void OverviewPage::setWalletModel(WalletModel* model)
{
    ...
    if (model && model->getOptionsModel()) {
        ...
        connect(ui->blabel_SATC, SIGNAL(clicked()), this, SLOT(openMyAddresses()));

    }

    // update the display unit, to not use the default ("SWYFT(SATC)")
    updateDisplayUnit();
}
...
void OverviewPage::updatBlockChainInfo()
{
 if (masternodeSync.IsBlockchainSynced())
 {
    ...
    double BlockRewardSATC =  static_cast<double>(BlockReward)/static_cast<double>(COIN);

    ui->label_CurrentBlock_value->setText(QString::number(CurrentBlock));
    ui->label_CurrentBlockReward_value->setText(QString::number(BlockRewardSATC));
...
void OverviewPage::updateObfuscationProgress()
{
    ...
    QString strAnonymizeSATCAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeSATCAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if (currentBalance == 0) {
        ...
        strAnonymizeSATCAmount = strAnonymizeSATCAmount.remove(strAnonymizeSATCAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeSATCAmount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    ...
    if (nMaxToAnonymize > nAnonymizeSATCAmount * COIN) nMaxToAnonymize = nAnonymizeSATCAmount * COIN;

    if (nMaxToAnonymize == 0) return;

    if (nMaxToAnonymize >= nAnonymizeSATCAmount * COIN) {
 //       ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
   //                                           .arg(strAnonymizeSATCAmount));
        strAnonymizeSATCAmount = strAnonymizeSATCAmount.remove(strAnonymizeSATCAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeSATCAmount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    } else {
