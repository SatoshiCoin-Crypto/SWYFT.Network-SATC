...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeSATCAmount"))
        settings.setValue("nAnonymizeSATCAmount", 1000);

    nObfuscationRounds = settings.value("nObfuscationRounds").toLongLong();
    nAnonymizeSATCAmount = settings.value("nAnonymizeSATCAmount").toLongLong();
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeSATCAmount:
            return QVariant(nAnonymizeSATCAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case ObfuscationRounds:
            ...
            break;
        case AnonymizeSATCAmount:
            nAnonymizeSATCAmount = value.toInt();
            settings.setValue("nAnonymizeSATCAmount", nAnonymizeSATCAmount);
            emit anonymizeSATCAmountChanged(nAnonymizeSATCAmount);
            break;
        case CoinControlFeatures:
